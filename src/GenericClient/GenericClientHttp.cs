...
namespace PepperDash.Essentials.Plugin.TriplePlay.IptvServer
{
    ...
    {        
        ...
        public GenericClientHttp(string key, EssentialsControlPropertiesConfig controlConfig)
        {
            ...
            Debug.LogVerbose("{0}", new String('-', 80));
            Debug.LogVerbose("GenericClient: Key = {0}", Key);
            Debug.LogVerbose("GenericClient: Host = {0}", Host);
            Debug.LogVerbose("GenericClient: Port = {0}", Port);
            Debug.LogVerbose("GenericClient: Username = {0}", Username);
            Debug.LogVerbose("GenericClient: Password = {0}", Password);
            Debug.LogVerbose("GenericClient: AuthorizationBase64 = {0}", AuthorizationBase64);
            ...
            Debug.LogVerbose("clientUrl: {0}", _clientHttp.Url.ToString());
        ...
        public void SendRequest(string requestType, string path, string content)
        {
            ...
            if (_clientHttp.ProcessBusy)
            {
                _requestQueue.Enqueue(() => _clientHttp.DispatchAsync(request, (response, error) =>
                {
                    if (response == null)
                    {
                        Debug.LogDebug("DispatchRequest: response is null, error: {0}", error);
                        return;
                    }
        ...
        public void SendRequest(string request, string contentString)
        {
            ...
            {
                Debug.LogDebug( "SendRequest: request is null or empty");
                Debug.LogError("SendRequest: request is null or empty");
                return;
            }
        ...
        {
            Debug.LogDebug("OnResponseReceived: args.Code = {0} | args.ContentString = {1}", args.Code,
                args.ContentString);
        ...
        {
            Debug.LogVerbose("CheckRequestQueue: _requestQueue.Count = {0}", _requestQueue.Count);
            var nextRequest = _requestQueue.TryToDequeue();
            Debug.LogVerbose("CheckRequestQueue: _requestQueue.TryToDequeue was {0}",
        ...
        private string EncodeBase64(string username, string password)
        {
            ...
            {
                Debug.LogVerbose("EncodeBase64 Exception:\r{0}", err);
                Debug.LogError("EncodeBase64 Exception:\r{0}", err);
                return "";
            }
        }
    }
